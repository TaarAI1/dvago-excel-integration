...
async def _is_qty_note_processed_in_oracle(note: str, oc: dict) -> bool:
    """Return True if Oracle has a finalized qty adjustment (adj_type=0) with this note."""
...
async def _process_note_batch(
    ...
    if note and not existing_doc_id:
        try:
            if await _is_qty_note_processed_in_oracle(note, oc):
                logger.info("[QtyAdj] Note '%s' already processed in RetailPro — skipping", note)
            ...
            logger.warning("[QtyAdj] Could not check duplicate note '%s' in Oracle: %s", note, guard_exc)
